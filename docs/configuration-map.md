...
```
See [install/AGENTS.md](../install/AGENTS.md) for module details.
...
- `codex/*.config.toml` → `~/.codex/*.config.toml` (named profiles copied, not symlinked)
- `codex/agents/*.toml` → `~/.codex/agents/*.toml` (individual agent symlinks via `install/codex.sh`)
- `codex/skills/*` → `~/.agents/skills/*` (Codex-specific skill symlinks via `install/codex.sh`)
