...
When a project directory has a `.fizzy.yml`, prefer the boards it declares over picking from `fizzy boards list`. If no mapping exists, resolve the target from the request or list candidates. Ask only when it remains ambiguous.
